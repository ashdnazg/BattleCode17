...
public class Soldier {
    ...
    boolean tryFire(RobotInfo nextEnemyInfo, MapLocation nextEnemy, RobotType enemyType, float dist, float radius) throws GameActionException {
        ...
        Radio.reportContact();
        if ((dist - radius < 1.51 + Math.max(0, money / 70f - 2) + Math.max(0, 4 * nearbyEnemySoldiers - 3) || !Util.tooManyTrees && dist < 7 && enemyType == RobotType.SOLDIER && (money > 60.0f || rc.getTreeCount() > 2)) && (maxArc > PENTAD_ARC_PLUSMINUS || dist < 3) && rc.canFirePentadShot()) {
