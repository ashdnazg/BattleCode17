...
public class Scout {
    ...
    RobotController rc;
    johnny4_sprint.Map map;
    Radio radio;
    johnny4_sprint.Movement movement;
    boolean isShaker;
    ...
    public Scout(RobotController rc) {
        ...
        this.movement = new johnny4_sprint.Movement(rc);
    ...
    void setAggro(boolean value) {
        ...
        isAggro = value;
        if (isAggro) {
            johnny4_sprint.Movement.MIN_ENEMY_DIST = 5.5f;
            johnny4_sprint.Movement.MIN_ENEMY_SCOUT_DIST = 0f;
            johnny4_sprint.Movement.MIN_ENEMY_LUMBERJACK_DIST = 4.5f;
        }
        if (!isAggro) {
            johnny4_sprint.Movement.MIN_ENEMY_DIST = 5.5f;
            johnny4_sprint.Movement.MIN_ENEMY_SCOUT_DIST = 0f;
            johnny4_sprint.Movement.MIN_ENEMY_LUMBERJACK_DIST = RobotType.LUMBERJACK.bodyRadius + RobotType.SCOUT.bodyRadius + GameConstants.LUMBERJACK_STRIKE_RADIUS + RobotType.LUMBERJACK.strideRadius;
        }
    }
    ...
    protected void tick() {
        try {
            ...
            if (Util.DEBUG)
                System.out.println("mindist: " + johnny4_sprint.Movement.MIN_ENEMY_DIST);
            int frame = rc.getRoundNum();
            radio.frame = frame;
            MapLocation myLocation = rc.getLocation();
            ...
            if (nextCivilianInfo != null && nextCivilianInfo.type == RobotType.SCOUT) {
                johnny4_sprint.Movement.MIN_ENEMY_SCOUT_DIST = 0;
                    ...
                    if (!longRangeCiv && dist < attackDistance) {
                        ...
                        if (!hasFired) {
                            if (checkLineOfFire(myLocation, nextCivilian, trees, nearbyRobots, RobotType.SCOUT.bodyRadius) && Util.fireAllowed && rc.canFireSingleShot()) {
                                ...
                                rc.fireSingleShot(fireDir);
                                hasFired = true;
                                johnny4_sprint.Movement.lastLOS = frame;
