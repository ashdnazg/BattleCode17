...
import johnny4_sprint.*;
import johnny4_sprint.Util;

import static johnny4_sprint.Util.*;
...
public class Movement {
    ...
    public Movement(RobotController rc) {
        ...
        if (johnny4_sprint.Util.DEBUG) System.out.println("min angle for " + robotType + " is " + MIN_MOVE_TO_FIRE_ANGLE);
    }

    // Call this every frame before using
    public static void init(RobotInfo[] robots_, TreeInfo[] trees_, BulletInfo[] bullets_) {
        if (johnny4_sprint.Util.DEBUG) System.out.println("Starting init " + Clock.getBytecodeNum());
        ...
        if (noEnemies) { // only keep distance to lumberjacks in combat
            threatsLen = 0;
            if (johnny4_sprint.Util.DEBUG) System.out.println("No threats, hugging friendly lumberjacks");
        }
        if (myLocation.distanceTo(stuckLocation) > strideDistance * 1.1) {
            stuckLocation = myLocation;
            stuckSince = lastInit;
        }
        if (johnny4_sprint.Util.DEBUG) System.out.println("ending init " + Clock.getBytecodeNum());
    }
    ...
    public boolean findPath(MapLocation target, Direction fireDir) throws GameActionException {
        if (johnny4_sprint.Util.DEBUG) System.out.println("Starting findPath " + Clock.getBytecodeNum());
        if (target == null) {
            if (johnny4_sprint.Util.DEBUG) System.out.println("Pathfinding to null, that's easy");
            return false;
        }
        ...
        if (DEBUG) {
            if (johnny4_sprint.Util.DEBUG) System.out.println("Pathfinding to " + target + "(dist: " + target.distanceTo(myLocation) + ", dir: " + myLocation.directionTo(target) + ") avoiding bullet in dir " + fireDir);
        }
        this.fireDir = fireDir;

        if (johnny4_sprint.Util.DEBUG) System.out.println("Somewhere in findPath " + Clock.getBytecodeNum());
        ...
        if (olddist < 0.0001f){
            if (valueMove(Direction.getNorth(), 0) < 0.0001 && !gonnaBeHit) {
                if (johnny4_sprint.Util.DEBUG) System.out.println("I'm already at target");
                return false;
            }else{
                if (johnny4_sprint.Util.DEBUG) System.out.println("I'm already at target but it sucks being around here");
        ...
        if (johnny4_sprint.Util.DEBUG) rc.setIndicatorLine(myLocation, target, 0, 0, 255);
        if (robotType != RobotType.SCOUT && olddist > 0.9 * strideDistance) {
            ...
            if (best != null) {
                ...
                if (Math.abs(moveDir.radiansBetween(toTree)) < correctionAngle) {
                    if (DEBUG) {
                        if (johnny4_sprint.Util.DEBUG) System.out.println("Found blocking tree at angle " + bestval);
                    }
                    if (johnny4_sprint.Util.DEBUG) rc.setIndicatorLine(myLocation, best.location, 255, 0, 0);
                    ...
                    if (johnny4_sprint.Util.DEBUG) rc.setIndicatorLine(myLocation, myLocation.add(moveDir, (float) Math.sqrt(myLocation.distanceSquaredTo(best.location) + (robotType.bodyRadius + best.radius) * (robotType.bodyRadius + best.radius))), 0, 255, 0);
                }
            }
        }

        if (johnny4_sprint.Util.DEBUG) System.out.println("Somewhere else in findPath " + Clock.getBytecodeNum());
        if (olddist > 2 * strideDistance && lastInit - stuckSince > 4) {
            if (DEBUG) {
                if (johnny4_sprint.Util.DEBUG) System.out.println("Switching bugdir because of stuck");
            }
            stuckSince = rc.getRoundNum();
            bugdir = !bugdir;
        }
        ...
        if (DEBUG) {
            if (johnny4_sprint.Util.DEBUG) System.out.println(olddist + " -> " + myLocation.distanceTo(target) + " : " + retval);
        }
        if (retval && olddist < myLocation.distanceTo(target) && (olddist < GO_STRAIGHT_DISTANCE || lastLOS >= rc.getRoundNum() - 1 && olddist < GO_STRAIGHT_DISTANCE * 2.5)) {
            if (DEBUG) {
                if (johnny4_sprint.Util.DEBUG) System.out.println("Switching bugdir because of distance");
            }
            bugdir = !bugdir;
        }
        if (johnny4_sprint.Util.DEBUG) System.out.println("end of findPath " + Clock.getBytecodeNum());
        return retval;
    ...
    private boolean bugMove(Direction dir, float dist) {
        if (johnny4_sprint.Util.DEBUG) System.out.println("Start bugmove: " + Clock.getBytecodeNum());
        float ret = LJ_tryMove(dir, 52, 3, bugdir, dist);
        if (ret >= 180) {
            bugdir = !bugdir;
            if (johnny4_sprint.Util.DEBUG) System.out.println("Switching bugdir to evade enemy");
        }
        if (rand() < 0.02f || ret < 0 && rand() < 0.2f) {
            bugdir = !bugdir;
            if (johnny4_sprint.Util.DEBUG) System.out.println("Switching bugdir");
        }
        if (johnny4_sprint.Util.DEBUG) System.out.println("end bugmove: " + Clock.getBytecodeNum());
        return ret > -0.01f;
    }
