...
public class HexagonalClusters extends Grid {
    ...
    public MapLocation getNearestWalkableLocation(MapLocation myLocation) {
        float OFFSET = johnny4_seeding.HexagonalClusters.OFFSET;
        int len = PATTERN.length;
        MapLocation best = null;
        float bestDist = 1000000;
        float dist;
        if (johnny4_seeding.Util.DEBUG) System.out.println(OFFSET);
        try {
            float radius = 5;
    ...
    public MapLocation getNearestPlantableLocation(MapLocation myLocation, TreeInfo[] treeInfos) {
        ...
        try {
            ...
            if (best != null)
                if (johnny4_seeding.Util.DEBUG) rc.setIndicatorDot(best, 0, 255, 0);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
