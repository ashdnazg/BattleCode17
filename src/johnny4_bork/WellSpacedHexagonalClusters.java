...
import static johnny4_bork.Util.rand;
            ...
            for (gx = Math.max(0,(int) ((myLocation.x - radius) / SPACING)); gx < xmax; gx++) {
                ...
                for (; gy < ymax; gy += PATTERN_SKIP_INVERSE_X[gy % P_Y]) {
                    ...
                    if ((!canSense || rc.onTheMap(checkLocation))) {
                        if (johnny4_bork.Util.DEBUG) rc.setIndicatorDot(realLocation, 0, 0, 255);
        ...
        try {
            ...
            for (gx = (int) Math.max(0, (myLocation.x - radius) / SPACING); gx < xmax; gx++) {
                ...
                for (; gy < ymax; gy += PATTERN_SKIP_X[gy % P_Y]) {
                    ...
                    if (johnny4_bork.Util.DEBUG) rc.setIndicatorDot(realLocation, 255, 255, 0);
            ...
            if (best != null)
                if (johnny4_bork.Util.DEBUG) rc.setIndicatorDot(best, 0, 255, 0);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
