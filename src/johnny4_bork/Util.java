...
import static johnny4_bork.Radio.*;
...
public class Util {
    ...
    static boolean checkLineOfFire(MapLocation start, MapLocation target, TreeInfo[] trees, RobotInfo robots[], float shooterRadius) {
        ...
        if (cnt >= px.length) {
            if (johnny4_bork.Util.DEBUG) System.out.println("Exhausted array length");
        }
        if (mindist > 99999d) {
            if (johnny4_bork.Util.DEBUG) System.out.println("No collision found");
            if (cnt >= px.length) {
                return false;
            }
        }
    ...
    static MapLocation predict(RobotInfo enemy, RobotInfo lastEnemy) throws GameActionException {
        ...
        if (lastEnemy != null && lastEnemy.getID() == enemy.getID()) {
            ...
            if (johnny4_bork.Util.DEBUG) System.out.println(time + ": " + dx + "|" + dy);
            if (johnny4_bork.Util.DEBUG) System.out.println("From " + lastEnemy.location + " to " + enemy.location);
            nextEnemy = new MapLocation(enemy.location.x + dx * time, enemy.location.y + dy * time);
            if (johnny4_bork.Util.DEBUG) rc.setIndicatorLine(lastEnemy.location, enemy.location, 255, 255, 0);
            if (johnny4_bork.Util.DEBUG) rc.setIndicatorLine(enemy.location, nextEnemy, 255, 100, 0);
        }
        if (johnny4_bork.Util.DEBUG) rc.setIndicatorDot(nextEnemy, 255, 100, 0);
        return nextEnemy;
    }
    ...
    static TreeInfo[] senseBiggestTrees() throws GameActionException {
        if (rc.getRoundNum() % 3 == 0 || cache.length == 0) {
            ...
            if (time > 300) {
                if (johnny4_bork.Util.DEBUG) System.out.println("Sensing trees took " + time);
            }
            if (cache.length >= temp.length) {
                if (johnny4_bork.Util.DEBUG) System.out.println("Reached maximum tree count");
            }
        }
        return cache;
    }
    ...
    static RobotInfo[] senseClosestEnemies() throws GameActionException {
        ...
        time = Clock.getBytecodeNum() - time;
        if (time > 300) {
            if (johnny4_bork.Util.DEBUG) System.out.println("Sensing robots took " + time);
        }
        if (cache3.length >= temp3.length) {
            if (johnny4_bork.Util.DEBUG) System.out.println("Reached maximum robots count");
        }
        return cache3;
    }
