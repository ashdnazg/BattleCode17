...
public class TreeStorage {
    ...
    public static void updateTrees(TreeInfo trees[]) throws GameActionException {
        ...
        for (int i = 0; i < knownTrees.length; i++) {
            if (treeHealth[i] > 0) {
                if (!_updated[i] && rc.canSenseLocation(knownTrees[i])) {
                    treeHealth[i] = 0;
                    ownTree[i] = false;
                    if (johnny4_bork.Util.DEBUG) rc.setIndicatorDot(knownTrees[i], 255, 0, 0);
                } else {
                    storedTrees++;
                    if (ownTree[i]) {
                        ownTrees++;
        ...
        if (time > 1000) {
            if (johnny4_bork.Util.DEBUG) System.out.println("Updating trees took " + time);
            if (johnny4_bork.Util.DEBUG) System.out.println(time1 + " " + time2 + " " + time3 + " " + time4);
        }
    }
    ...
    public static MapLocation waterTree(RobotInfo[] nearbyRobots) throws GameActionException {
        ...
        time = Clock.getBytecodeNum() - time;
        if (time > 1000) {
            if (johnny4_bork.Util.DEBUG) System.out.println("Getting nearest waterable location took " + time);
        }
        toWater = best;
        if (best >= 0) {
